...
def wm_mask_verifier(wm_mask):
    ...
    ###
    post_sum = np.sum(pad_mask[...])
    
    #This should be 0 if the cropping did not affect the WM mask.
    vox_diff = np.abs(initial_sum - post_sum)
...
def do_unet_2017(input_dir,
    ...
    num_not_redone = 0
    num_miss_n4 = 0
    missing_id_text_append = "These are missing: "
    
    n = len(id_list_dir)
    n_i = 1
    
    #Readying model; outside of loop to only do it once:
    img_shape = (rows_standard, cols_standard, flair+t1)
        
    model = get_unet(img_shape, True)
    model_path = '/home/lars/Desktop/micca2017/weights_h5/0_final.h5' #This must be changed manually.
    ...
    for i in id_list_dir:
        ...
        FLAIR_filename = input_dir + str(i) + '/' + 'T2_FLAIR_3D/t2_flair_3d.nii.gz'
        
        if not os.path.isfile(FLAIR_filename):
            print "Skipping: " + str(i) + " because its FLAIR image is missing."
            missing_id_text_append = missing_id_text_append + " " + str(i) + " FLAIR"
        ...
        T1_filename = input_dir + str(i) + '/' + 'T1_3D_SAG/t1_3d_sag.nii.gz'
        
        if not os.path.isfile(T1_filename):
            print "Skipping: " + str(i) + " because its T1w image is missing."
            missing_id_text_append = missing_id_text_append + " " + str(i) + " T1"
            num_miss_n4 = num_miss_n4 + 1
        ...
        print("Saving Shell world matrix call...")
        ...
        if do_wm_masks:
            ...
            print("Saving Shell WM masking call...")
            print(apply_wm_mask_command)
        ...
        print("Updating records of processed IDs...")
    ...
    print "Number missing T1/FLAIR images: " + str(num_miss_n4) + " of " + str(n)
    
    if num_miss_n4 > 0:
        print("One or more images in pairs were missing. Your intended calculations may not be complete.")
        print(missing_id_text_append)
        #End-if
...
##Running:
test_directory = "/home/lars/Desktop/wmh_thirty_cases_article/unet_wm_post_run/test_data/"
...
out_calls = '/home/lars/Desktop/wmh_thirty_cases_article/unet_wm_post_run/other/shell_call_n4_transformation_fslmaths_30.txt'
finished_list = '/home/lars/Desktop/wmh_thirty_cases_article/unet_wm_post_run/other/processed_IDs.txt' 
c_or_r = 'redo'
do_wm = True
wm_diagnostics = '/home/lars/Desktop/wmh_thirty_cases_article/unet_wm_post_run/other/WM_diagnostics.txt'
wm_masking_call = '/home/lars/Desktop/wmh_thirty_cases_article/unet_wm_post_run/other/shell_call_wm_masking_fslmaths_30.txt' 
